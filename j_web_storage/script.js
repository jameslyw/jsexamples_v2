...
async function fetchColorsList() {
    ...
    try {
        ...
        let page = resp.page + 1; // move to page 2 after the 1st dataset
        ...
        for (let currentPage = page; currentPage <= totalPages; currentPage++) {
            ...
            const resp = await response.json();
            allColors.push(...resp.data);      // spread operator: the returned array to be pushed into allColors array
        }
        ...
        errHeader.className = "d-flex justify-conent text-danger py-2";
